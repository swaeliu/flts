...
from curriculum.config import ReLoRAConfig  # noqa: E402

from .dp import DP_MODES  # noqa: E402
...
class GVendiConfig(ReLoRAConfig):
    ...
    # ------------------------------------------------------------------
    # FedChronos-style clip + Gaussian-noise perturbation of the LoRA
    # update (NOT full-pipeline client-level DP -- the forecast head is
    # still averaged unperturbed; see gvendi/dp.py). Off by default; the
    # training loop is byte-identical to the current code path when off.
    # ------------------------------------------------------------------
    gvendi_dp_mode: str = "off"     # {"off","clip_only","noise_only","dp"}
    gvendi_dp_epsilon: float = 8.0  # FedChronos-matched per-round noise-
                                     # calibration parameter -- NOT an
                                     # end-to-end privacy budget
    gvendi_dp_delta: float = 1e-5
    gvendi_dp_clip_norm: float = 1.0  # C; fixed experimental hyperparameter,
                                       # never auto-rescaled at runtime
    gvendi_dp_seed: int = 46
...
def add_gvendi_cli_args(parser) -> None:
    ...
    p.add_argument("--gvendi_dp_mode", type=str, default=None,
                   choices=list(DP_MODES))
    p.add_argument("--gvendi_dp_epsilon", type=float, default=None)
    p.add_argument("--gvendi_dp_delta", type=float, default=None)
    p.add_argument("--gvendi_dp_clip_norm", type=float, default=None)
    p.add_argument("--gvendi_dp_seed", type=int, default=None)
...
GVENDI_OVERRIDE_NAMES = [
    ...
    "gvendi_dp_mode", "gvendi_dp_epsilon", "gvendi_dp_delta",
    "gvendi_dp_clip_norm", "gvendi_dp_seed",
]
...
def validate_gvendi_config(
    ...
    if cfg.gvendi_dp_mode not in DP_MODES:
        raise ValueError(
            f"gvendi_dp_mode must be one of {DP_MODES}, got {cfg.gvendi_dp_mode!r}"
        )
    if cfg.gvendi_dp_epsilon <= 0.0:
        raise ValueError("gvendi_dp_epsilon must be positive")
    if not (0.0 < cfg.gvendi_dp_delta < 1.0):
        raise ValueError("gvendi_dp_delta must lie in (0, 1)")
    if cfg.gvendi_dp_clip_norm <= 0.0:
        raise ValueError("gvendi_dp_clip_norm must be positive")
